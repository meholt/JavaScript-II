...
runners.forEach(function(currentObj, first_name, last_name) {
    const name = `${currentObj.first_name} ${currentObj.last_name}`;
    fullName.push(name);
})
...
runners.forEach(function(currentObj, first_name) {
    let firstName = `${currentObj.first_name}`;
...
let largeShirts = runners.filter((currentObj) => {
   return  currentObj.shirt_size === 'L';
});
